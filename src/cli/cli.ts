...
import { ProjectConfig } from '../generator/user-inputs/prompt-answers';
...
const promptUser = async (): Promise<ProjectConfig> => {
    ...
    if (baseAnswers.fullStack === 'Yes') {
      const fsQuestions = extensionManager.getGenerator(baseAnswers.selectedBackEnd).getRequiredInputs();
