...
async function prepAndRunEnvironmentTf(jobDetails, rootFolderPath, action) {
  ...
  const environmentModulePath = './terraform-modules/azure/environment';
  ...
  const backend = {
    ...
    key: `utopiops-water/applications/environment/${jobDetails.details.environmentName}` // TODO: This must be unique, somehow centralize it
  }
  ...
  const runTerraformOptions = {
    ...
    tfVars: {
      ...
      domain: JSON.stringify(antiNullEnv.domain || {}).replace(/":/g, '"='),
      location: antiNullEnv.location,
      enable_vnet_ddos_protection: antiNullEnv.enableVnetDdosProtection
    }
  }
