...
async function prepAndRunProviderTf(jobDetails, rootFolderPath, action) {
  ...
  const providerModulePath = './terraform-modules/azure/provider';
  ...
  const runTerraformOptions = {
    ...
    action,
    tfVars: {
      location: jobDetails.details.location,
      resource_group_name: jobDetails.details.resourceGroupName,
      storage_account_name: jobDetails.details.storageAccountName,
    }
  }
