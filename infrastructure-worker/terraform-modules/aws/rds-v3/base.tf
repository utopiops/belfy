locals {
  db_identifier        = format("%s-%s", var.environment, var.name)
  master_password      = var.create_random_password ? random_password.master_password[0].result : var.password
  db_subnet_group_name = !var.cross_region_replica && var.replicate_source_db != null ? null : coalesce(var.db_subnet_group_name, module.db_subnet_group.db_subnet_group_id, local.db_identifier)
...
module "db_subnet_group" {
  ...
  name            = coalesce(var.db_subnet_group_name, local.db_identifier)
...
module "db_parameter_group" {
  ...
  name            = coalesce(var.parameter_group_name, local.db_identifier)
...
module "db_option_group" {
  ...
  name                     = coalesce(var.option_group_name, local.db_identifier)
