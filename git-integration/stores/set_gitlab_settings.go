...
// SetGitlabApplicationSettings sets the application service provider and gitlab application settings.
func (store *settingsStore) SetGitlabApplicationSettings(ctx context.Context, accountID, environmentName, applicationName, integrationName, projectID string) (err error) {
...
INSERT INTO gitlab_application_settings (project_id)
...
`
		...
		var settingsId int
		err = tx.QueryRow(insertGitlabApplicationSettings, projectID).Scan(&settingsId)
		if err != nil {
			return
		}
