...
// SetGithubApplicationSettings sets the application service provider and github application settings.
func (store *settingsStore) SetGithubApplicationSettings(ctx context.Context, accountID, environmentName, applicationName, integrationName, repoFullName string) (err error) {
...
`
			...
			// WHERE id = $1
			// `
			insertGithubApplicationSettings = `
INSERT INTO github_application_settings (repo_full_name)
...
`
		...
		var settingsId int
		err = tx.QueryRow(insertGithubApplicationSettings, repoFullName).Scan(&settingsId)
		if err != nil {
			return
		}
