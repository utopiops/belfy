...
func (store *settingsStore) UpdateGitlabApplicationSettings(ctx context.Context, accountID, environmentName, applicationName, integrationName, projectID string) (err error) {
...
`
			...
			updateGitlabApplicationSettings = `
UPDATE gitlab_application_settings
SET project_id = $1
WHERE id = $2
`
...
`
		...
		// Get settings_id for update gitlab_application_settings table
		var settingsId int
		...
		_, err = tx.Exec(updateGitlabApplicationSettings, projectID, settingsId)
		if err != nil {
			return
		}
