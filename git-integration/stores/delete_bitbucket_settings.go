...
func (store *settingsStore) DeleteBitbucketApplicationSettings(ctx context.Context, accountID, environmentName, applicationName string) (err error) {
...
`
			...
			deleteBitbucketApplicationSettings = `
DELETE FROM bitbucket_application_settings
WHERE id = $1
`
...
`
		...
		// Get settings_id for delete from bitbucket_application_settings table
		var settingsId int
		...
		_, err = tx.Exec(deleteBitbucketApplicationSettings, settingsId)
		if err != nil {
			return
		}
