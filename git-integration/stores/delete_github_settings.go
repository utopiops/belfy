...
func (store *settingsStore) DeleteGithubApplicationSettings(ctx context.Context, accountID, environmentName, applicationName string) (err error) {
...
`
			...
			deleteGithubApplicationSettings = `
DELETE FROM github_application_settings
WHERE id = $1
`
...
`
		...
		// Get settings_id for delete from github_application_settings table
		var settingsId int
		...
		_, err = tx.Exec(deleteGithubApplicationSettings, settingsId)
		if err != nil {
			return
		}
