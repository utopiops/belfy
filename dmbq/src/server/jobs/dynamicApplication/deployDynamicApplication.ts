...
async function jobPromise(details: any, body: any): Promise<object> {
  return new Job({
    ...
    url: `${config.coreUrl}/v3/applications/environment/name/${body.environmentName}/application/name/${body.applicationName}/deploy`,
    method: 'POST',
    body: {
      variables: {
    ...
    filter: {
      ...
      dynamicNames: { $elemMatch: { name: body.dynamicName, 'state.code': 'deployed' } },
    },
    details: {
      isDynamic: true,
