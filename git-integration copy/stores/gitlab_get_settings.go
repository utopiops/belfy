...
func (store *settingsStore) GetGitlabSettings(ctx context.Context, accountID, environmentName, applicationName string) (setting *models.GitlabSettings, err error) {
...
JOIN gitlab_application_settings gas ON asp.settings_id = gas.id
WHERE account_id = $1 AND environment_name = $2 AND application_name = $3
`
	}

	s := models.GitlabSettings{}
