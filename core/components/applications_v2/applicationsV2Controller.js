...
exports.getProvidersWithSameBackend = getProvidersWithSameBackend;
...
async function createEnvironment(req, res, next) {
  ...
  const validationSchema = yup.object().shape({
    ...
    providerName: yup.string().required()
      .oneOf(['aws'], "Accepted values: aws"),
    parentHostedZoneName: yup.string().required(),
    isOwn: yup.boolean().required(),
    isCrossAccount: yup.boolean().required()
  });
  try {
  ...
  const { name, region, description, providerName, providerId, parentHostedZoneName, isOwn, isCrossAccount } = req.body;
  ...
  try {
    if (providerName !== constants.applicationProviders.aws) { // At the moment we only support AWS
      res.sendStatus(constants.statusCodes.badRequest);
      return;
    }
    let result = await AwsEnvironment.add({ accountId, userId, name, region, description, providerName, tfCodePath, providerId, parentHostedZoneName, isOwn, isCrossAccount });
    if (!result.success) {
      if (result.message == constants.errorMessages.models.duplicate) {
        res.sendStatus(constants.statusCodes.duplicate);
        return;
      }
      res.status(constants.statusCodes.ise).send(result.message);
      return;
    } else {
...
async function listEnvironment(req, res) {
  const accountId = tokenService.getAccountIdFromToken(req);
  try {
    let result = await EnvironmentModel.getAll(accountId);
    if (!result.success) {
      if (result.message == constants.errorMessages.models.duplicate) {
        res.sendStatus(constants.statusCodes.duplicate);
        return;
      ...
      const environments = result.output.environments;
      // Get the status and effects of the alarms of the environments
      const alarmsResult = await EnvironmentAlarmModel.listEnvironmentsAlarmStatus(environments.map(environment => environment.id))
      if (alarmsResult.success) {
        const alarmsObject = alarmsResult.output.alarms.reduce((acc, curr) => { acc[curr._id] = curr.alarms; return acc; }, {})
        const environmentsWithAlarms = [...environments];
...
async function getEnvironmentProviderCredentials(req, res) {
  res.send({ credentials: res.locals.credentials });
}
//-----------------------------------------
async function getProvidersWithSameBackend(req, res) {
  const environmentName = req.params.name;
  const accountId = tokenService.getAccountIdFromToken(req);
  // Check if the environment belongs to the user and get it's provider's details
  const envProvider = await getEnvironmentIdAndProviderName(accountId, environmentName, res);
  if (!envProvider) {
    return;
  }
  try {
    const result = await Provider.getSameTypeProviders(accountId, envProvider.providerName, envProvider.providerDisplayName);
    if (!result.success) {
      res.sendStatus(constants.statusCodes.badRequest);
      return;
    }
    res.send({ providers: result.output.providers })
  } catch (error) {
...
async function tfActionApplication(action, req, res) {
  ...
  try {
    if (action === 'deploy' || action === 'destroy') {
      ...
      const result = await EnvironmentApplicationModel.setState(environmentId, applicationName, state);
      if (!result.success) {
        res.status(constants.statusCodes.badRequest).send(`Application is ${action}ing now. Please wait.`)
        return
      }
    }
  } catch (error) {
...
async function getApplicationResources(req, res, next) {
  ...
  const { bucketName, region } = result.output.provider;
  let credentials = {
    accessKeyId: res.locals.credentials.accessKeyId,
    secretAccessKey: res.locals.credentials.secretAccessKey
  }
