...
module.exports = {
  createEksBackgroundJobApplication,
  addEksBackgroundJobApplicationVersion,
  updateEksBackgroundJobApplication,
  createPipeline
}

async function createEksBackgroundJobApplication(environmentId, environmentName, accountId, userId, app_name, description, isDynamicApplication, appVersion, credentials, region, cloudProviderAccountId, bucketName, headers) {
  let step = 0;
  let appId;
  try {
      const newApp = {
          ...
          activeVersion: 1,
          deployedVersion: 1,
          releaseName: `${ app_name }-${ uuidv4().slice(0, 8) }`,
      };
      ...
      const doc = new EksBackgroundJobApplication(appVersion);
      await doc.save();
      step++;
      ...
      await createPipeline(accountId, userId, environmentId, app_name, region, cloudProviderAccountId, credentials, repositoryName, newApp.releaseName, headers);
      return {
          success: true,
      ...
      try {
          ...
          if (step > 1) {
              ...
              await EksBackgroundJobApplication.findOneAndDelete({ _id: appId }).exec();
          }
      } catch (e) {
...
//-------------------------------------
async function addEksBackgroundJobApplicationVersion(environmentId, app_name, description, appVersion, fromVersion) {
  let step = 0;
  let appId;
  try {
...
//-------------------------------------
async function updateEksBackgroundJobApplication(environmentId, app_name, description, appVersion) {
  try {
      // Check if the environment application exists (get it's _id)
      const filter = {
          environment: environmentId,
      ...
      const doc = await EksBackgroundJobApplication.findOneAndUpdate(appVersionFilter, appVersion, { new: true }).exec();
      if (doc == null) {
          return {
              success: false,
              error: {
...
//---------------------------------------------------------------------------------------
async function createPipeline(accountId, userId, environmentId, applicationName, region, cloudProviderAccountId, credentials, repositoryName, releaseName, headers) {
    try {
      ...
      const appVersion = await EksBackgroundJobApplication.findOne(appVersionFilter).exec();
      if (appVersion == null) {
        return {
          error: {
            message: constants.errorMessages.models.elementNotFound,
