...
const providerSchema = new Schema({
  ...
  state: {
    ...
    default: {
        code: constants.resourceStatus.deploying,
    }
  },
  accountId: {
    type: ObjectId
...
azureProviderSchema.virtual('decryptedCredentials').get(function () {
  return {
    subscription: decrypt(this.subscription),
    appId: decrypt(this.appId),
    tenant: decrypt(this.tenant),
    password: decrypt(this.password),
  };
});

azureProviderSchema.virtual('summary').get(function () {
  return {
    name: this.name,
    region: this.region,
...
const digitalOceanProviderSchema = new Schema({
  region: String,
...
digitalOceanProviderSchema.virtual('decryptedCredentials').get(function () {
  return {
    digitalOceanToken: decrypt(this.digitalOceanToken),
    spacesAccessKeyId: decrypt(this.spacesAccessKeyId),
    spacesSecretAccessKey: decrypt(this.spacesSecretAccessKey),
  };
});

digitalOceanProviderSchema.virtual('summary').get(function () {
  return {
    name: this.name,
    region: this.region,
    bucketName: this.bucketName
...
const gcpProviderSchema = new Schema(
  {
    region: String,
...
gcpProviderSchema.virtual('decryptedCredentials').get(function () {
  return {
    serviceAccountKey: decrypt(this.serviceAccountKey),
  };
});

gcpProviderSchema.virtual('summary').get(function () {
  return {
    name: this.name,
    region: this.region,
    bucketName: this.bucketName,
...
providerSchema.path('backend').discriminator(constants.applicationProviders.digitalOcean, digitalOceanProviderSchema);
providerSchema.path('backend').discriminator(constants.applicationProviders.gcp, gcpProviderSchema);
